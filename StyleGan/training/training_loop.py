...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(7680 // training_set.image_shape[2], 7, 6)  # 保存中间过程图，6*6=36张图片
    gh = np.clip(4320 // training_set.image_shape[1], 4, 6)
    print('gw:', gw, 'gh:', gh)
...
# ----------------------------------------------------------------------------
##This entire function performs the process of training the model
def training_loop(
    ...
    G = dnnlib.util.construct_class_by_name(**G_kwargs, **common_kwargs).train().requires_grad_(False).to(
        device)  # G is for Generator. subclass of torch.nn.Module
    D = dnnlib.util.construct_class_by_name(**D_kwargs, **common_kwargs).train().requires_grad_(False).to(
        device)  # D is for Discriminator. subclass of torch.nn.Module
    ...
    if rank == 0:
        z = torch.empty([batch_gpu, G.z_dim], device=device)  # (b,512) G.z_dim=512
        c = torch.empty([batch_gpu, G.c_dim], device=device)  # (b,2) G.c_dim=2
        img = misc.print_module_summary(G, [z, c])  # (b,c,h,w)
        # print(img.shape, img.dtype)
        # for i in range(len(img)):
        #     print(img[i].cpu().numpy().transpose([1, 2, 0]).shape)
        #     img[i] = torch.from_numpy(cv.cvtColor(img[i].cpu().numpy().transpose([1, 2, 0]), cv.COLOR_BGRA2BGR))
        #     print(img[i].shape)
        #     cv.imshow('img', img[i].cpu().numpy())
        #     cv.waitKey()
        # img =
    ...
    if rank == 0:
        ...
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(batch_gpu)  # ((16,512),(16,512),(4,512))
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)  # ((16,2),(16,2),(4,2))
        images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        save_image_grid(images, os.path.join(run_dir, 'fakes_init.png'), drange=[-1, 1], grid_size=grid_size)  # Save intermediate process map
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_c = [phase_gen_c.split(batch_gpu) for phase_gen_c in all_gen_c.split(batch_size)]  # (48,2)所有标签
        ...
        # Update state.
        # previous_cur_nimg = cur_nimg  # Record the previous cur_nimg
        cur_nimg += batch_size
        batch_idx += 1
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
            save_image_grid(images, os.path.join(run_dir, r'result_image', f'fakes{cur_nimg // 1000:06d}.png'),
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            if rank == 0:
                with open(snapshot_pkl, 'wb') as f:
                    ...
                    try:  # Delete the saved weights from the previous step
                        os.remove(os.path.join(run_dir,
                                               f'network-snapshot-{cur_nimg // 1000 - image_snapshot_ticks:06d}.pkl'))
                    except:
                        pass
        # Evaluate metrics.
