...
def modulated_conv2d(
        x,  # (16,512,36,36)  # Input tensor: [batch_size, in_channels, in_height, in_width]
        w,  # (512,512,3,3)  # Weight tensor: [out_channels, in_channels, kernel_height, kernel_width]
        s,  # (16,512)  # Style tensor: [batch_size, in_channels]
    ...
    out_channels, in_channels, kh, kw = w.shape  # (512,512,3,3)
    ...
    if demodulate:
        w = w * w.square().mean([1, 2, 3], keepdim=True).rsqrt()  # (512,512,3,3)
        s = s * s.square().mean().rsqrt()  # (16,512)

    # Modulate weights.
    w = w.unsqueeze(0)  # (1,512,512,3,3)  # [NOIkk]
    w = w * s.unsqueeze(1).unsqueeze(3).unsqueeze(4)  # (b,512,512,3,3)   # [NOIkk]
    ...
    x = x.reshape(batch_size, -1, *x.shape[2:])  # (b,c,h,w) (16,512,38,38)
    return x
...
class Generator(torch.nn.Module):
    def __init__(self,
                 z_dim,  # Input latent (Z) dimensionality.
                 c_dim,  # Conditioning label (C) dimensionality. 需要先转为one-hot编码！
                 w_dim,  # Intermediate latent (W) dimensionality.
...
if __name__ == '__main__':
    ...
    batch_gpu = 1
    labels = np.array([0])
    # G = Generator(z_dim=512, c_dim=-1, w_dim=512, img_resolution=128, img_channels=3).cuda()
    weight_path = r'G:\stylegan3(pytorch)\output_dir\00006-stylegan3-t-123-gpus1-batch16-gamma12.2\network-snapshot-000200.pkl'  # 训练好的权重所在路径，比如：stylegan3-r-afhqv2-512x512.pkl
    with open(weight_path, 'rb') as f:
        G = pickle.load(f)['G_ema'].eval().cuda()  # torch.nn.Module
    z = torch.randn([1, G.z_dim]).cuda()  # latent codes
    c = torch.tensor([0]).cuda()  # class labels (not used in this example)
    c = F.one_hot(c, G.c_dim)
    imgs = G(z, c)  # NCHW, float32, dynamic range [-1, +1], no truncation
    print(imgs.shape)
    # grid_z = torch.randn([batch_gpu, G.z_dim]).cuda().split(batch_gpu)  # ((16,512),(16,512),(4,512))
    # grid_c = torch.from_numpy(labels).cuda().split(batch_gpu)
    # print(grid_z, grid_c)
