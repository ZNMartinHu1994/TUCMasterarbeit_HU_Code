...
dir_path = r'C:/work/data/1.original_dataset/%s/bad' % dataset_name  # Input folder path
save_dir = r'C:/work/generated_images/2.CycleGAN/type1/test_from_dirA2B'  # Output folder path
...
def gen_images_from_dir(gen_image_range, save_dir):
    ...
    for test_img in test_db:  # Input each image into the model for conversion
        ...
        with torch.no_grad():  # Test Generator
            ...
            gen_img = G_AB(test_img.float().to(device)).detach().cpu().numpy()
